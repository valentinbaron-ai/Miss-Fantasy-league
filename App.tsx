...
// Fix: Explicitly extending Component to avoid type resolution issues with React.Component
...
const Toast = ({ message, onClose }: { message: string, onClose: () => void }) => {
  ...
  return (
    <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[100] animate-slide-up w-full px-4 max-w-sm">
      <div className="bg-white text-black px-6 py-4 rounded-xl font-bold text-xs uppercase tracking-widest shadow-[0_0_30px_rgba(212,175,55,0.4)] flex items-center justify-center gap-3 border-2 border-premium-gold text-center">
...
const UnlockPodiumModal = ({ onClose, onConfirm }: { onClose: () => void, onConfirm: () => void }) => {
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/90 backdrop-blur-sm transition-opacity" onClick={onClose} />
      <div className="bg-zinc-900 w-full max-w-sm rounded-2xl border border-red-500/50 p-6 relative shadow-2xl animate-slide-up">
        
        <div className="text-center mb-6">
           <div className="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center mx-auto mb-3 text-3xl border border-red-500/30">
             🔓
           </div>
           <h3 className="text-white font-serif font-bold text-lg uppercase">Déverrouiller le Podium ?</h3>
        </div>

        <div className="bg-red-950/30 p-4 rounded-xl border border-red-500/20 mb-6">
           <p className="text-red-200 text-sm text-center leading-relaxed">
             Modifier votre podium validé coûtera <br/>
             <strong className="text-white text-lg">2 POINTS</strong> <br/>
             de pénalité à la prochaine validation.
           </p>
        </div>

        <div className="flex gap-3">
           <button onClick={onClose} className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-zinc-400 font-bold rounded-xl text-xs uppercase tracking-wider transition">
             Annuler
           </button>
           <button onClick={onConfirm} className="flex-1 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl text-xs uppercase tracking-wider shadow-lg shadow-red-900/40 transition">
             Confirmer
           </button>
        </div>
      </div>
...
function MainContent() {
  ...
  const [userDataLoading, setUserDataLoading] = useState(false); // Nouvel état de chargement des données utilisateur
  ...
  const [toastMessage, setToastMessage] = useState<string | null>(null); // State pour le Toast
  const [showUnlockModal, setShowUnlockModal] = useState(false); // State pour confirmation déblocage
  ...
  // NOUVEL ÉTAT POUR LA CONFIRMATION
  const [showFoucaultMessage, setShowFoucaultMessage] = useState(false);
  // ÉTAT DE CHARGEMENT SAUVEGARDE
  ...
  const loadUserData = async () => {
    if (!user) return;
    setUserDataLoading(true); // Début du chargement
    try {
        ...
        // Récupération sécurisée des paris
        const { data: bets, error: betsError } = await supabase.from('bets').select('*').eq('user_id', user.id);
        
        if (betsError) {
            console.error("Erreur chargement bets:", betsError);
        }
        ...
        if (bets) {
          ...
          if (podiumBet) {
            ...
            // Si des IDs sont présents, on verrouille pour éviter l'édition accidentelle
            if (podiumBet.candidate_ids && podiumBet.candidate_ids.length > 0) {
                setPodiumLocked(true);
            }
          }
        }
    } catch (e) {
        console.error("Erreur globale loadUserData:", e);
    } finally {
        setUserDataLoading(false); // Fin du chargement
    }
  };
  ...
  const handleSaveSelection = async (type: 'SQUAD' | 'PODIUM', ids: string[]) => {
    ...
    try {
        ...
        // 1. Vérifier si un vote existe déjà manuellement (évite erreur 42P10 upsert)
        ...
        if (type === 'SQUAD') {
           setSquad({ id: existingBet?.id || 'saved', userId: user.id, type: 'SQUAD', candidateIds: ids, timestamp: Date.now() });
           setShowSelectionRecap(true);
           setToastMessage("✅ Top 12 validé et sauvegardé !");
        }
        if (type === 'PODIUM') {
           setPodium({ id: existingBet?.id || 'saved', userId: user.id, type: 'PODIUM', candidateIds: ids, timestamp: Date.now(), modificationCount: modCount });
           setPodiumLocked(true);
           setShowFoucaultMessage(true);
           setToastMessage("✅ Podium validé et sauvegardé !");
        }
    } catch (e: any) {
        console.error("Erreur sauvegarde:", e);
        // Affiche le message d'erreur détaillé ou l'objet stringifié
  ...
  // MODIFICATION ICI : Au lieu de window.confirm, on ouvre la modale
  const handleUnlockPodiumRequest = () => {
      setShowUnlockModal(true);
  };

  const confirmUnlockPodium = () => {
      setPodiumLocked(false);
      setShowUnlockModal(false);
      setToastMessage("🔓 Podium débloqué pour modification");
  };
  ...
  // AFFICHAGE DU CHARGEMENT DES DONNÉES UTILISATEUR
  if (userDataLoading) {
      return (
        <div className="min-h-screen bg-black flex flex-col items-center justify-center">
            <div className="w-8 h-8 border-4 border-premium-gold border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="text-premium-gold text-xs font-bold uppercase tracking-widest animate-pulse">Récupération de vos pronostics...</p>
        </div>
      );
  }
  ...
  return (
    <div className="min-h-screen bg-premium-dark text-white pb-24">
      ...
      <div className="pt-20 px-4 max-w-md mx-auto">
        ...
        {currentScreen === 'Squad' && (
          <div className="animate-slide-up pb-20">
             ...
             <div className="sticky top-20 z-30 bg-black/80 backdrop-blur-md p-4 rounded-xl border border-premium-gold/30 mb-4 flex justify-between items-center shadow-xl">
                  ...
                  <span className={`text-2xl font-serif font-bold ${squad?.candidateIds.length === 12 ? 'text-green-500' : 'text-premium-gold'}`}>{(squad?.candidateIds.length || 0)} / 12</span>
             </div>

             {squad?.candidateIds.length === 12 && (
                <div className="space-y-4 mb-6">
                  <HostVoice text="Vos 12 favorites sont prêtes pour la demi-finale ! Pensez à valider." />
                </div>
             )}
             ...
             <div className="grid grid-cols-2 gap-2">
                {candidates.map(c => {
                    ...
                    // Si on a 12 sélectionnés et que ce n'est pas celui-ci, on le grise légèrement
                    const isDisabled = !isSelected && (squad?.candidateIds.length || 0) >= 12;
                    
                    return (
                        <div key={c.id} className={isDisabled ? 'opacity-40 grayscale transition-all' : 'opacity-100 transition-all'}>
                            <MissCard 
                                candidate={c} 
                                selected={isSelected} 
                                disabled={false} // On laisse clickable pour désélectionner ou "tenter" de sélectionner (logique gérée ci-dessous)
                                badge={officialResults.top15.length > 0 ? (officialResults.top15.includes(c.id) ? '✓ QUALIFIÉE' : '✕ ÉLIMINÉE') : undefined} 
                                onPress={() => {
                                    const current = squad?.candidateIds || [];
                                    if (current.includes(c.id)) { 
                                        // Désélection
                                        setSquad(prev => ({ ...prev!, candidateIds: current.filter(id => id !== c.id) } as PlayerSelection)); 
                                    } else if (current.length < 12) { 
                                        // Sélection
                                        setSquad({ id: 'temp', userId: user.id, type: 'SQUAD', candidateIds: [...current, c.id], timestamp: 0 }); 
                                    } else {
                                        // Tentative d'ajout alors que plein
                                        alert("Vous avez déjà 12 candidates ! Retirez-en une pour changer.");
                                    }
                                }} 
                            />
                        </div>
             ...
             {(squad?.candidateIds.length || 0) === 12 && (
                <div className="fixed bottom-24 left-4 right-4 animate-slide-up z-30">
                   <PrimaryButton 
                        title={saveLoading ? "Validation en cours..." : "VALIDER MES 12"} 
                        onPress={() => handleSaveSelection('SQUAD', squad!.candidateIds)} 
                        disabled={saveLoading}
                        variant="gold" 
                        className="shadow-2xl shadow-premium-gold/30 border-2 border-white/20" 
                   />
                </div>
             )}
          </div>
                    ...
                    {podiumLocked ? (
                        <div className="flex flex-col gap-2">
                             <div className="bg-black/80 border border-white/20 p-4 rounded-xl text-center backdrop-blur-md shadow-2xl">
                                ...
                                <button onClick={handleUnlockPodiumRequest} className="w-full py-3 bg-red-600 hover:bg-red-500 text-white rounded-lg text-xs font-bold uppercase transition shadow-lg animate-pulse border border-red-400">DÉBLOQUER (COÛT : -2 PTS)</button>
                             </div>
                        </div>
                    ) : (
                        (podium?.candidateIds || []).filter(Boolean).length === 5 && (
                           <PrimaryButton 
                               title={saveLoading ? "Validation en cours..." : (podium?.modificationCount || 0) > 0 ? "VALIDER (MISE À JOUR)" : "VALIDER MA SÉLECTION (DÉFINITIF)"} 
                               onPress={() => handleSaveSelection('PODIUM', podium!.candidateIds || [])} 
                               disabled={saveLoading}
                               variant="gold" 
                               className="shadow-2xl shadow-premium-gold/30" 
                           />
                        )
                    )}
                  </div>
                      ...
                      <div className="fixed inset-0 z-[60] flex flex-col bg-zinc-900/95 backdrop-blur-xl animate-slide-up">
                          ...
                          <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 gap-3 content-start">
                              {candidates.filter(c => squad.candidateIds.includes(c.id)).map(c => {
                                  const usedIndex = podium?.candidateIds?.indexOf(c.id);
                                  const isUsed = usedIndex !== undefined && usedIndex !== -1 && usedIndex !== selectingSlot;
                                  
                                  let badgeText = undefined;
                                  if (isUsed) {
                                      if (usedIndex === 0) badgeText = "MISS FRANCE";
                                      else if (usedIndex === 1) badgeText = "1ÈRE DAUPHINE";
                                      else if (usedIndex === 2) badgeText = "2ÈME DAUPHINE";
                                      else if (usedIndex === 3) badgeText = "3ÈME DAUPHINE";
                                      else if (usedIndex === 4) badgeText = "4ÈME DAUPHINE";
                                      else badgeText = "DÉJÀ SÉLECTIONNÉE";
                                  }

                                  return (
                                    <div key={c.id} className={isUsed ? 'opacity-50 grayscale' : ''}>
                                        <MissCard 
                                            candidate={c}
                                            selected={false}
                                            badge={badgeText}
                                            ...
                                            onPress={() => {
                                                ...
                                                const basePodium = podium || { 
                                                    id: 'temp', 
                                                    userId: user.id, 
                                                    type: 'PODIUM', 
                                                    candidateIds: Array(5).fill(''), 
                                                    timestamp: 0 
                                                };
                                                setPodium({ 
                                                    ...basePodium, 
        ...
        {/* ... (Teams, TeamDetail, Rules screens remain same) */}
        {currentScreen === 'Teams' && (
          <div className="animate-slide-up pb-20">
        ...
        {showFoucaultMessage && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 animate-slide-up">
             <div className="absolute inset-0 bg-black/90 backdrop-blur-md" onClick={() => setShowFoucaultMessage(false)} />
             <div className="bg-gradient-to-br from-france-blue to-black border border-premium-gold/50 p-6 rounded-2xl relative z-10 max-w-sm text-center shadow-[0_0_50px_rgba(212,175,55,0.2)]">
                <div className="w-16 h-16 bg-white rounded-full mx-auto -mt-12 mb-4 border-4 border-premium-gold flex items-center justify-center text-4xl shadow-lg">
                   🎙️
                </div>
                <h3 className="text-premium-gold font-serif font-bold text-xl mb-2">Jean-Pierre Foucault</h3>
                <p className="text-white italic text-lg leading-relaxed">
                   "Félicitations pour votre choix, <br/> <span className="text-premium-gold font-bold">Jimmy</span> fera le délibéré !"
                </p>
                <button onClick={() => setShowFoucaultMessage(false)} className="mt-6 bg-white text-france-blue font-bold px-6 py-2 rounded-full uppercase text-xs tracking-widest hover:bg-zinc-200 transition">
                   Merci Jean-Pierre
                </button>
             </div>
          </div>
        ...
        {/* MODALE DE CONFIRMATION DE DÉBLOCAGE */}
        {showUnlockModal && (
          <UnlockPodiumModal onClose={() => setShowUnlockModal(false)} onConfirm={confirmUnlockPodium} />
        )}
